...
function main() {
  ...
  assertContains(catalogDataArtistsSource, "artistNameNormExpr = getArtistNameNormalizationExpr(\"a\", columns)", "getArtistBySlug uses normalized-name expression for all-short-term slug fallback", failures);
  assertContains(catalogDataArtistsSource, "ensureArtistSearchPrefixIndex()", "getArtistBySlug ensures prefix index before normalized-prefix fallback scans", failures);
