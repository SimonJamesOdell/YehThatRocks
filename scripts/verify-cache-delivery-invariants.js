...
function main() {
  const failures = [];
  const nginxConfig = readFileStrict(files.nginxConfig, ROOT);
  ...
  finishInvariantCheck({
    failures,
    failureHeader: "Cache delivery invariant check failed:\n",
    successMessage: "Cache delivery invariant check passed.",
  });
}
