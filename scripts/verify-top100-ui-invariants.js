...
const files = {
  ...
  top100Link: path.join(ROOT, "apps/web/components/leaderboard-video-link.tsx"),
...
function main() {
  ...
  assertContains(top100LinkSource, "<p className=\"leaderboardVideoCategory\"><VideoGenreLink genre={categoryLabel} stopPropagation className=\"leaderboardCategoryLink\" /></p>", "Leaderboard card renders category label as deep-link metadata above artist-track text", failures);
  ...
  assertContainsOneOf(
    ...
    "Top 100 warmed link warms on click",
    failures,
  );
  ...
  assertContains(currentVideoRouteSource, "relatedVideos.length < RESOLVE_CURRENT_VIDEO_TARGET_RELATED_COUNT", "Current video route pads sparse related list", failures);
  assertContains(currentVideoRouteSource, "RESOLVE_CURRENT_VIDEO_TARGET_RELATED_COUNT = 8;", "Current video route pads Watch Next up to 8 items", failures);
