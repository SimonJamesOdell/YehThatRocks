...
import {
  ...
  ARTIST_CATALOG_EVIDENCE_CACHE_TTL_MS,
  ARTIST_HEAVY_CACHE_MAX_ENTRIES,
  ARTIST_LETTER_CACHE_TTL_MS,
  ARTIST_LETTER_PAGE_CACHE_TTL_MS,
  ARTIST_NORM_VIDEO_POOL_CACHE_TTL_MS,
  ARTIST_NORM_VIDEO_POOL_HEADROOM_ROWS,
  ARTIST_NORM_VIDEO_POOL_MAX_ROWS,
  ARTIST_NORM_VIDEO_POOL_MIN_ROWS,
  ARTIST_PROJECTION_REFRESH_TTL_MS,
  ARTIST_SEARCH_CACHE_TTL_MS,
  ARTIST_SINGLE_SLUG_CACHE_TTL_MS,
  ARTIST_SLUG_LOOKUP_CACHE_TTL_MS,
  ARTIST_STATS_LETTER_BACKFILL_TTL_MS,
  ARTIST_VIDEO_METADATA_SEARCH_CACHE_TTL_MS,
  ARTIST_VIDEOS_CACHE_TTL_MS,
  ARTISTS_LIST_CACHE_TTL_MS,
  KNOWN_ARTIST_MATCH_CACHE_TTL_MS,
  SAME_GENRE_RELATED_POOL_CACHE_TTL_MS,
} from "@/lib/artist-constants";
...
export async function getArtistVideoPoolByNormalizedName(normalizedArtist: string, limit: number): Promise<RankedVideoRow[]> {
  ...
  const resolvePool = async () => {
    ...
    const details = await prisma.$queryRawUnsafe<Array<{
      ...
      `
        SELECT v.id, v.videoId, v.title, COALESCE(NULLIF(TRIM(v.parsedArtist), ''), NULLIF(TRIM(v.channelTitle), ''), NULL) AS channelTitle, NULLIF(TRIM(v.parsedArtist), '') AS parsedArtist, NULLIF(TRIM(v.parsedTrack), '') AS parsedTrack, NULLIF(TRIM(v.genre), '') AS genre, v.favourited, v.description
        FROM videos v
    ...
    const detailById = new Map(details.map((row: { id: number; videoId: string; title: string; channelTitle: string | null; parsedArtist: string | null; parsedTrack: string | null; genre: string | null; favourited: number | null; description: string | null }) => [row.id, row]));
    ...
    const rows = await prisma.$queryRawUnsafe<RankedVideoRow[]>(
      `
        SELECT /*+ MAX_EXECUTION_TIME(800) */
          v.videoId, v.title, COALESCE(NULLIF(TRIM(v.parsedArtist), ''), NULLIF(TRIM(v.channelTitle), ''), NULL) AS channelTitle, NULLIF(TRIM(v.parsedArtist), '') AS parsedArtist, NULLIF(TRIM(v.parsedTrack), '') AS parsedTrack, NULLIF(TRIM(v.genre), '') AS genre, v.favourited, v.description
        FROM videos v
        WHERE v.videoId IS NOT NULL
          AND COALESCE(v.approved, 0) = 1
