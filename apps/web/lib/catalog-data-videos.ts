...
const VIDEO_CACHE_MAX_ENTRIES = Math.max(
  200,
  Math.min(8_000, Number(process.env.VIDEO_CACHE_MAX_ENTRIES || "1000")),
);
    ...
    const rows = mode === "top"
      ...
      : await prisma.$queryRaw<Array<{ videoId: string }>>`
          ...
          WHERE sv.status = 'available'
            AND v.videoId IS NOT NULL
            AND COALESCE(v.approved, 0) = 1
          GROUP BY v.id, v.videoId, v.created_at
          ORDER BY v.created_at DESC, v.id DESC
          LIMIT ${fetchLimit}
        `;

    const safeRows = Array.isArray(rows) ? rows : [];
    const ids = Array.from(new Set<string>(safeRows.map((row: { videoId: string | null }) => row.videoId).filter((id: string | null): id is string => Boolean(id)))).slice(0, fetchLimit);
