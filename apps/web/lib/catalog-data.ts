...
async function getArtistVideoPoolByNormalizedName(normalizedArtist: string, limit: number): Promise<RankedVideoRow[]> {
  ...
  const resolvePool = async () => {
    ...
    // Keep the first phase narrow so MySQL can satisfy most work via the artist norm covering index.
...
async function hydrateArtistCountryByName(artist: ArtistRecord): Promise<ArtistRecord> {
  if (!hasDatabaseUrl() || artist.country !== "Unknown") {
    return artist;
  }
  ...
  try {
    ...
    const rows = await prisma.$queryRawUnsafe<Array<{ country: string | null }>>(
      `
        SELECT a.${countryCol} AS country
        FROM artists a
        WHERE a.${nameCol} = ?
        LIMIT 1
      `,
...
async function hasArtistStatsProjection() {
  ...
  try {
    ...
    // Some deployments disallow SHOW COLUMNS while regular SELECT is permitted.
    // Probe the projection table directly before falling back to expensive artist queries.
    try {
...
async function hasArtistStatsThumbnailColumn() {
  ...
  try {
    ...
    // SHOW COLUMNS may be blocked even when selecting known columns is allowed.
    try {
  ...
  for (let offset = 0; offset < validRows.length; offset += BATCH_UPSERT_SIZE) {
    ...
    if (hasThumbnailColumn) {
      ...
      for (const row of batch) {
        params.push(
          row.normalizedArtist, row.displayName, row.slug, row.firstLetter,
...
function scheduleArtistStatsLetterBackfill(letter: string, rows: Array<ArtistRecord & { videoCount: number }>) {
  ...
  const promise = (async () => {
    await batchUpsertArtistStatsRows(rows, "runtime-letter-backfill");
...
async function getVideoArtistNormalizationColumn() {
  ...
  try {
    ...
    // Some deployments restrict SHOW COLUMNS, which can force expensive LOWER/TRIM
    // fallbacks even when parsed_artist_norm exists. Probe the hot-path column directly.
    try {
...
export async function getNewestVideos(
  ...
  const resolveNewestVideos = async () => {
    ...
    const tryNewestFastPath = async () => {
      // Keep the optimization bounded so large custom offsets still use canonical SQL below.
      ...
      for (let rawOffset = 0; rawOffset < maxRawScan && collected.length < desiredAvailableWindow; rawOffset += batchSize) {
    ...
    try {
      if (await hasArtistStatsProjection()) {
        ...
        const thumbnailSelect = hasThumbnailColumn
          ? "s.thumbnail_video_id AS thumbnailVideoId"
...
export async function getVideosByArtist(artistName: string, limit = 500) {
  ...
  const normalizedArtist = normalizeArtistKey(exactArtist);
